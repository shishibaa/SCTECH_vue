  ...
  <Panel title="ลูกค้า">
    ...
    <div class="overflow-x-auto">
      <table class="w-full text-sm">
        <thead><tr class="bg-slate-50 text-slate-600"><th class="th">ชื่อ</th><th class="th">เบอร์</th><th class="th">อีเมล</th><th class="th">เลขผู้เสียภาษี</th><th class="th w-32">จัดการ</th></tr></thead>
        <tbody>
    ...
    <dialog ref="dlg" class="rounded-2xl p-0 w-[min(520px,90vw)]">
      <form @submit.prevent="save" class="bg-white rounded-2xl border border-slate-200 shadow-lg overflow-hidden">
        <header class="px-4 py-3 border-b border-slate-200 font-semibold text-[#0F3460]">{{ editing?.id ? 'แก้ไขลูกค้า' : 'เพิ่มลูกค้า' }}</header>
        <div class="p-4 space-y-3">
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              ...
              <p v-if="editing.phone && !/^0\\d{8,9}$/.test(editing.phone)" class="form-error">รูปแบบเบอร์ไม่ถูกต้อง</p>
            </div>
            <div>
