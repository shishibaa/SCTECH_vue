<template>
  <Panel title="ใบเสนอราคา">
    <div class="flex items-center justify-between mb-3">
      ...
      <button class="btn-primary" @click="openCreate()">+ สร้างใบเสนอราคา</button>
    </div>

    <div class="overflow-x-auto">
      <table class="w-full text-sm">
        <thead><tr class="bg-slate-50 text-slate-600"><th class="th">เลขที่</th><th class="th">ลูกค้า</th><th class="th">วันที่</th><th class="th">สถานะ</th><th class="th text-right">สุทธิ</th><th class="th w-40">จัดการ</th></tr></thead>
        <tbody>
          <tr v-for="c in filtered" :key="c.id" class="border-b last:border-b-0">
            <td class="td">{{ c.doc_no || '(ร่าง)' }}</td>
            <td class="td">{{ c.customer_name }}</td>
            <td class="td">{{ c.doc_date }}</td>
            <td class="td">{{ c.status || 'DRAFT' }}</td>
    ...
    <dialog ref="dlg" class="rounded-2xl p-0 w-[min(860px,96vw)]">
      <form @submit.prevent="save" class="bg-white rounded-2xl border border-slate-200 shadow-lg overflow-hidden">
        <header class="px-4 py-3 border-b border-slate-200 flex items-center justify-between">
          <span class="font-semibold text-[#0F3460]">{{ editing?.id ? 'แก้ไขใบเสนอราคา' : 'สร้างใบเสนอราคา' }}</span>
          <label class="inline-flex items-center gap-2 text-sm"><input type="checkbox" v-model="editing.include_vat"/> รวม VAT ({{ editing.vat_rate }}%)</label>
        </header>
        <div class="p-4 space-y-4">
          <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
            <div>
              <label class="form-label">ลูกค้า</label>
              <input v-model.trim="editing.customer_name" class="form-input" placeholder="พิมพ์ชื่อชั่วคราว" required />
            </div>
            <div>
            ...
            <div>
              ...
              <input v-model.trim="editing.doc_no" class="form-input" placeholder="เช่น QUO-202509-0001" />
            </div>
          </div>
          ...
          <div class="flex flex-col sm:flex-row gap-3 justify-end">
            <div class="rounded-xl bg-slate-50 px-4 py-3 min-w-[260px]">
              ...
              <div class="flex justify-between text-sm"><span>VAT ({{ editing.include_vat ? editing.vat_rate : 0 }}%)</span><span>{{ fmt(vat) }}</span></div>
              <div class="flex justify-between font-semibold mt-1"><span>ยอดสุทธิ</span><span>{{ fmt(total) }}</span></div>
            </div>
          </div>
        </div>
        <footer class="px-4 py-3 border-t border-slate-200 flex justify-between bg-slate-50">
          <div class="text-sm text-slate-600">สถานะ: <strong>{{ editing.status }}</strong></div>
          <div class="flex gap-2">
...
onMounted(async () => { list.value = await api.get('/quotations') || [] })
...
const vat      = computed(() => editing.value?.include_vat ? subtotal.value * (Number(editing.value.vat_rate || 0) / 100) : 0)
...
async function save() {
  ...
  if (payload.id) {
    const saved = await api.put('/quotations', payload)
    const idx = list.value.findIndex(x => x.id === saved.id); if (idx > -1) list.value[idx] = saved
    toast.success('บันทึกแล้ว')
  } else {
    const saved = await api.post('/quotations', payload)
    list.value.push(saved); toast.success('สร้างใบเสนอราคาแล้ว')
  }
  dlg.value.close()
}

async function remove(c) { if (!confirm('ลบเอกสารนี้?')) return; await api.del(`/quotations/${c.id}`); list.value = list.value.filter(x => x.id !== c.id); toast.info('ลบแล้ว') }
</script>